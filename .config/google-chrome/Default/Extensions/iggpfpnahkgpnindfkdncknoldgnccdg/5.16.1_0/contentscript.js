...
getUrlVars: function(name) {
  ...
  if (results == null) {
    results = new RegExp('[\?&]' + name + '=?([^&#]*)?').exec(top.location.href);
...
checkIf: function (c) {
  switch (c) {
    ...
    case 'url-loop': return (ytl.getUrlVars('loop') == 0 ? true : false);
...
setCountInputPanel: function () {
  ...
  var countCheckbox;
  if (ytl.layout == '2017' || ytl.layout == '2016') {
    countCheckbox = document.createElement('paper-checkbox');
  } else {
    countCheckboxContainer.className = 'yt-uix-form-input-checkbox-container';
    countCheckbox = document.createElement('input');
  ...
  countCheckbox.addEventListener ('click', function(e) { 
    ...
    ytl.replaceUrlVar('loop', (ytl.checkIf('inloop') ? '0' : null));
    ytl.checkAutoPlay();
    return;
  }, false);
...
setAutoLoop: function () {
  var autoLoopInterval = setInterval(function() {
    ...
    // 
    clickButton = false;
    if (ytl.getOption('saveStateLoop') || (ytl.checkIf('playlistExist') && ytl.checkIf('playlist-endplayPrevious'))) {
...
getReady: function () {
  try {
    ...
    } else if (!ytl.checkIf('playlistExist') && !ytl.checkIf('playlist-endplayPrevious')) {
      // Reset
      ytl.storage['yt-loop-playlist-endplay'] = false;
      ytl.storage['yt-loop-incount'] = false;
